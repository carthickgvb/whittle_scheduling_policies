function out = MWIS_algo3(n,G,M,psi,l,C,sim_time,inputs)
...
for slot = 1:sim_time
    idx = MWIS_neelay(n,A,X,psi);
     
    Cost(slot) = 0;
    for m=1:n
        Cost(slot) = Cost(slot) + C*X(m) + idx(m)*f(psi(m)); %including cost of transmission during collision
    end
