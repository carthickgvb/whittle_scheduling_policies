%Calculate Whittle Index for graphical Whittle Index based algorithm
function out = EquationSolve_graph(M,psi,C,l,mis)
...
for x = x_iter
    ...
    for n = 1:N
        ...
        lambda_old = lambda(n);
        lambda(n+1) = lambda(n) + gamma*(f(min(x,psi)) - lambda(n)*mis + temp_sum);
